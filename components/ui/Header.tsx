  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, flexShrink: 0 }}>
        ...
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
            <path d="M12 2L21.5 7.5v9L12 22l-9.5-5.5v-9L12 2z" stroke={t.accent} strokeWidth="1.5" strokeLinejoin="round"/>
            <path d="M12 8L17 11v6l-5 3-5-3v-6l5-3z" fill={t.accentBg} stroke={t.accent} strokeWidth="1" strokeLinejoin="round"/>
          </svg>
        </motion.div>
        <div>
          <div style={{
            ...
            System<span style={{ color: t.accent }}>Flow</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
        <HeaderChip label="Nodes" value={String(nodes.length)} color="var(--indigo)" />
        <HeaderChip label="Edges" value={String(edges.length)} color="var(--cyan)" />
      </div>
        ...
        {running && (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <LiveMetric label="LATENCY" value={`${Math.round(metrics.avgLatency)}ms`} color="var(--cyan)" />
              <LiveMetric label="P95"     value={`${metrics.percentiles?.p95 ?? 0}ms`}  color="var(--indigo)" />
              <LiveMetric label="THRUPUT" value={`${metrics.throughput.toFixed(1)}/s`}  color="var(--green)" />
              <LiveMetric label="ERR%"    value={`${metrics.errorRate.toFixed(1)}%`}     color={metrics.errorRate > 5 ? 'var(--red)' : 'var(--green)'} />
              <LiveMetric label="RATE"    value={`${liveRate}req/s`} color={t.accent} pulse />
            </div>
        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{
              ...
              background: user.avatarDataUrl ? 'transparent' : t.surface2,
              border: `1.5px solid ${showProfile ? t.accent : t.border2}`,
          ...
          >
            {user.avatarDataUrl ? (
              ...
              <span style={{ fontSize: 11, fontWeight: 800, color: t.textPrimary, fontFamily: 'var(--font-mono)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  <button
                    ...
                    style={{
                      ...
                      background: user.avatarDataUrl ? 'transparent' : t.surface2,
                      border: `2px solid ${t.border2}`, cursor: 'pointer',
                  ...
                  >
                    {user.avatarDataUrl ? (
                      ...
                      <span style={{ fontSize: 15, fontWeight: 800, color: t.textPrimary, fontFamily: 'var(--font-mono)' }}>
                        {user.username.slice(0, 2).toUpperCase()}
                      </span>
                    )}
                  </button>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {editingUsername ? (
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: t.textPrimary, fontFamily: 'var(--font-ui)' }}>
                          {user.username}
                        </span>
                        <button
        ...
        >
          {theme === 'dark' ? (
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
              <path d="M12 3a9 9 0 1 0 9 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 0 1-4.4 2.26 5.403 5.403 0 0 1-3.14-9.8c-.44-.06-.9-.1-1.36-.1z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          ) : (
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
