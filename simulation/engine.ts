...
import { analyzeArchitecture } from '@/utils/scoring';
...
export class SimulationEngine {
  ...
  validateArchitecture(): { valid: boolean; error?: string; warnings?: string[] } {
    if (this.nodes.size === 0) {
      return { valid: false, error: 'Architecture is empty. Add nodes to the canvas.' };
    }
    const userNodes = this.findUserNodes();
    if (userNodes.length === 0) {
      return { valid: false, error: 'No User/Client nodes found. Add a Client node to generate traffic.' };
    }
    ...
    const detectCycleDFS = (nodeId: string) => {
      if (!visited.has(nodeId)) {
        ...
        for (const neighbor of neighbors) {
          if (!visited.has(neighbor) && detectCycleDFS(neighbor)) {
            hasCycle = true;
    ...
    for (const nodeId of this.nodes.keys()) {
      if (!visited.has(nodeId) && detectCycleDFS(nodeId)) {
        break;
      }
    }

    if (hasCycle) {
      warnings.push('Cycle detected in the architecture. Requests may loop or drop unexpectedly.');
    }

    // 2. Detect unreachable nodes (DFS from user nodes)
    const reachable = new Set<string>();
    const reachableDFS = (nodeId: string) => {
      reachable.add(nodeId);
      const neighbors = this.adjacency.get(nodeId) || [];
      for (const neighbor of neighbors) {
        if (!reachable.has(neighbor)) reachableDFS(neighbor);
      }
    };
    for (const startNode of userNodes) {
      reachableDFS(startNode);
    }

    const unreachableCount = this.nodes.size - reachable.size;
    if (unreachableCount > 0) {
      warnings.push(`${unreachableCount} node(s) are unreachable from any Client node and will receive no traffic.`);
    }

    // 3. Ensure at least one valid path
    ...
    if (!hasValidPath) {
      return { valid: false, error: 'No complete paths found. Connect Client nodes to backend services.', warnings };
    }
  ...
  start(): boolean {
    ...
    if (!validation.valid) {
      this.emit({ type: 'alert', data: { message: validation.error } });
      return false;
    }

    if (validation.warnings && validation.warnings.length > 0) {
      this.emit({ type: 'alert', data: { message: 'Warning: ' + validation.warnings[0] } });
    }
  ...
  private computeHealthScore(avgLatency: number, errorRate: number): SystemHealthScore {
    const baseAnalysis = analyzeArchitecture(Array.from(this.nodes.values()), this.edges);
    let score = baseAnalysis.score;
    const suggestions: string[] = [...baseAnalysis.improvements];
    ...
    for (const [nodeId, node] of this.nodes) {
      ...
      if (nodeType.includes('lb') || nodeType.includes('load-balancer')) {
